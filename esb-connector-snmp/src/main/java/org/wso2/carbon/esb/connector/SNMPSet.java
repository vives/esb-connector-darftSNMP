				...
				if (jsonArray.length() > 0) {
					...
					while (jsonArray.length() > i) {
						...
						if (type.equalsIgnoreCase(SNMPConstants.STRING) ||
							...
							pdu.add(new VariableBinding(new OID(oid), new Counter32(
									Integer.parseInt(UpdateValue))));
						} else if ((type.equalsIgnoreCase(SNMPConstants.COUNTER64))) {
							pdu.add(new VariableBinding(new OID(oid), new Counter64(
									Integer.parseInt(UpdateValue))));
						}
						++i;
					}
				} else {
					handleException("The updates values are empty JSONArray.", messageContext);
				}
